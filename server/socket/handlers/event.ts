...
export function registerEventHandlers(io: Server, socket: Socket) {
	socket.on("admin:event:log", async (payload, ack) => {
		...
		io.to([`match:${event.matchId}`, `tournament:${event.tournamentId}`]).emit("match:event", event);
	...
	socket.on("admin:event:delete", async (payload, ack) => {
		...
		// Broadcast to relevant rooms (and keep legacy `eventId` for older clients).
		const payloadOut = { id: found.id, matchId: found.matchId, tournamentId: found.tournamentId, eventId: found.id };
		io.to([`match:${found.matchId}`, `tournament:${found.tournamentId}`]).emit("match:event:deleted", payloadOut);
		return ack?.({ ok: true });
	});
	...
	socket.on("stats:match:get", async (payload, ack) => {
		...
		const [events, stats] = await Promise.all([getMatchEvents(parsed.data.matchId), getMatchStats(parsed.data.matchId)]);
