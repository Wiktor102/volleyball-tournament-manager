...
export async function generateBracket(tournamentId: string, mode: "auto" | "manual" = "auto") {
	...
	// Add a dedicated third-place match (losers of semifinals) so it is always
	// visible in bracket views. Only meaningful when a semifinal round exists.
	if (rounds >= 2 && teams.length >= 4) {
		values.push({
			id: id("m"),
			tournamentId,
			roundNumber: rounds,
