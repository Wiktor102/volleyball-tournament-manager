...
function TeamCard({ team, onClick }: { team: Team; onClick: () => void }) {
	return (
		<button className="player-view-team-card" onClick={onClick}>
			<span className="player-view-team-color" style={{ background: team.color ?? "#64748b" }} />
...
function PlayerRow({ player }: { player: Player }) {
	...
	return (
		<div className="player-view-player-row">
			<span className="player-view-jersey">{player.jerseyNumber != null ? `#${player.jerseyNumber}` : "—"}</span>
			<span className="player-view-player-name">
				{player.name}
				{positionLabel && <span className="player-view-player-pos">{positionLabel}</span>}
			</span>
		</div>
	);
}
...
function LiveMatchBanner({
	...
	const isMyTeam =
		selectedTeamId != null && (currentMatch.team1Id === selectedTeamId || currentMatch.team2Id === selectedTeamId);
	...
	return (
		<div className={`player-view-live-banner${isMyTeam ? " player-view-live-banner--mine" : ""}`}>
			<div className="player-view-live-top">
				...
				<span className="player-view-live-label">{isMyTeam ? "TWOJA DRUŻYNA GRA TERAZ!" : "MECZ NA ŻYWO"}</span>
			</div>
...
function NextMatchBanner({
	...
	const isMyTeam =
		selectedTeamId != null && (nextMatch.team1Id === selectedTeamId || nextMatch.team2Id === selectedTeamId);
...
function BracketSummary({
	...
	const allMatches = [...recentMatches, ...upcomingMatches].filter(m => m.team1Id === teamId || m.team2Id === teamId);
	...
	if (allMatches.length === 0) {
		return <div className="player-view-bracket-empty">Brak danych o meczach dla tej drużyny.</div>;
	}
		...
		<div className="player-view-bracket">
			<div className="player-view-bracket-stats">
				<div className="player-view-bracket-stat">
					<span className="player-view-bracket-stat-val player-view-bracket-stat-val--win">{wins}</span>
					<span className="player-view-bracket-stat-label">Wygrane</span>
				</div>
				<div className="player-view-bracket-stat">
					<span className="player-view-bracket-stat-val player-view-bracket-stat-val--loss">{losses}</span>
								...
								<div className="player-view-bracket-outcome">
									...
									<span
										className={`player-view-outcome-badge ${
											won ? "player-view-outcome-badge--win" : "player-view-outcome-badge--loss"
										}`}
									>
...
export function PlayerView() {
	...
	const playersLoading = selectedTeamId != null && loadedTeamId !== selectedTeamId;
	...
	return (
		<div className="display-page">
			<div className="display-container">
				...
				{/* ── Live match banner ── */}
				<LiveMatchBanner currentMatch={currentMatch} selectedTeamId={selectedTeamId} teams={teams} score={score} />

				{/* ── Challenge banner ── */}
				{challenge &&
					(() => {
						const ct1 = teams.find(t => t.id === currentMatch?.team1Id);
						const ct2 = teams.find(t => t.id === currentMatch?.team2Id);
						return (
							<ChallengeBanner
								challenge={challenge}
								team1Name={ct1?.name ?? "D1"}
								team2Name={ct2?.name ?? "D2"}
								team1Color={ct1?.color}
								team2Color={ct2?.color}
							/>
						);
					})()}

				{/* ── Next match banner (only when team selected and it's theirs) ── */}
				{selectedTeamId && <NextMatchBanner nextMatch={nextMatch} selectedTeamId={selectedTeamId} teams={teams} />}
