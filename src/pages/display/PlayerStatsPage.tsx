...
export function PlayerStatsPage() {
	...
	const requestKey = socket && tournamentId && playerId ? `${tournamentId}:${playerId}` : "";
	...
	useEffect(() => {
		...
		socket.emit("stats:player:get", { tournamentId, playerId }, (ack: Ack<PlayerStats[]>) => {
		...
		<div className="display-page">
			<div className="display-container">
				<div className="display-header">
					<h1>{player?.name ?? "Zawodnik"} — Statystyki</h1>
					...
					<>
							...
							<div style={{ display: "flex", gap: 32, flexWrap: "wrap", alignItems: "flex-start" }}>
								<div className="stats-player-card">
									{player?.jerseyNumber != null && (
										<div className="stats-player-card__number">#{player.jerseyNumber}</div>
									)}
								...
								>
									{EVENT_LABELS.map(({ key, label }) => (
										<div key={key} className="stats-event-card">
											...
											<div className="stats-event-card__value">
												{ps?.stats[key as keyof typeof ps.stats] ?? 0}
											</div>
										</div>
									))}
						...
						{!ps && (
							<div className="card">
								<div className="empty-state">
									<div className="empty-state-icon">📊</div>
									<div className="empty-state-text">Brak zarejestrowanych zdarzeń dla tego zawodnika.</div>
								</div>
							</div>
						)}
					</>
