...
export function FanView() {
	...
	const hasNextMatch = nextMatch != null && nextMatch.team1Id != null && nextMatch.team2Id != null;
		...
		<div className="display-page">
			<div className="display-container">
				...
				{challenge && (
					<ChallengeBanner
						challenge={challenge}
						team1Name={team1?.name ?? "D1"}
						team2Name={team2?.name ?? "D2"}
				...
				{hasActiveMatch ? (
					...
					<div className="card">
						<div className="empty-state">
							...
							<div className="empty-state-text">Brak aktywnego meczu. Poczekaj na rozpoczęcie rozgrywki.</div>
						</div>
					</div>
				)}
				...
				{totalMatches > 0 && (
					<ModuleCard title="Postęp turnieju">
						<TournamentProgress completed={completedMatches} total={totalMatches} pct={progressPct} />
					</ModuleCard>
				)}
...
function ScheduleHistory({ upcoming, recent, teams }: { upcoming: MatchSummary[]; recent: MatchSummary[]; teams: Team[] }) {
	return (
...
function TournamentProgress({ completed, total, pct }: { completed: number; total: number; pct: number }) {
	return (
