...
export function TeamsManager() {
    ...
    const onState = (state: TournamentState) => {
      ...
      setDrafts((prev) => {
        ...
        for (const t of state.teams) {
          if (!next[t.id]) next[t.id] = { name: t.name, shortName: t.shortName ?? '', color: t.color ?? '#f97316' }
        }
        return next
      })
    }
  ...
  const cancelEdit = (teamId: string, team: Team) => {
    setDrafts((prev) => ({
      ...prev,
      [teamId]: { name: team.name, shortName: team.shortName ?? '', color: team.color ?? '#f97316' },
    }))
    setEditingId(null)
  }
      ...
      <div className="admin-container">
        ...
        <div className="card">
          ...
          {teams.length === 0 ? (
            ...
            <div className="list">
              {teams.map((t) => {
                const d = drafts[t.id] ?? { name: t.name, shortName: t.shortName ?? '', color: t.color ?? '#f97316' }
                    ...
                    {isEditing ? (
                      <>
                        ...
                        <div className="btn-group">
                          ...
                          <button className="btn btn-secondary btn-sm" onClick={() => cancelEdit(t.id, t)}>
                            Anuluj
                          </button>
                        </div>
                      </>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div className="color-preview" style={{ background: t.color || '#f97316', width: 24, height: 24 }}></div>
                          <div>
                            <div className="list-item-title" style={{ color: t.color || undefined }}>
                              {t.name}
                              {t.shortName && <span className="text-dim"> ({t.shortName})</span>}
                            </div>
                          </div>
                        </div>
                        <div className="btn-group">
                          <button className="btn btn-secondary btn-sm" onClick={() => setEditingId(t.id)}>
                            Edytuj
                          </button>
        ...
        {teams.length > 0 && teams.length < 2 && (
          <div className="info-message">
            Dodaj co najmniej 2 drużyny, aby móc wygenerować drabinkę turniejową.
          </div>
        )}
        {teams.length >= 2 && (
          <div className="info-message">
            Masz {teams.length} drużyn. Możesz teraz{' '}
            <Link to="/admin/bracket">wygenerować drabinkę →</Link>
          </div>
        )}
      </div>
    </div>
